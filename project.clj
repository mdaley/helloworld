(defproject helloworld "1.0.0"
  ...
  :ring {:handler helloworld.web/app
         :main helloworld.setup
         :port ~(Integer/valueOf (get (System/getenv) "SERVICE_PORT" "8080"))
         :init helloworld.setup/setup
  ...
  :rpm {:name "helloworld"
        ...
        :mappings [{:directory "/usr/local/helloworld"
                    :filemode "444"
                    :username "helloworld"
                    :groupname "helloworld"
                    :sources {:source [{:location "target/helloworld.jar"}]}}
                   {:directory "/usr/local/helloworld/bin"
                    :filemode "744"
                    :username "helloworld"
                    :groupname "helloworld"
                   ...
                   {:directory "/etc/rc.d/init.d"
                    :filemode "755"
                    :sources {:source [{:location "scripts/service/helloworld"
                                        :destination "helloworld"}]}}]}


  :aot [helloworld.setup]
