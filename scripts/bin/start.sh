...
if [ -n "$PIDS" ]
then
  echo "Helloworld is already running in process $PIDS";
  exit 1
fi
...
LOG_FILE=${LOGGING_PATH}/helloworld.out
ERR_FILE=${LOGGING_PATH}/helloworld.err
...
echo "Giving Helloworld $waitTimeout seconds to start successfully"
...
do
  ...
  then
    echo "Helloworld didn't start within $waitTimeout seconds."
    PIDS=$(pgrep java -lf | grep $APP_NAME | cut -d" " -f1);
    if [ -n "$PIDS" ]
	then
	  echo "Killing $PIDS";
...
if [ $retVal -eq 1 ]
then
  echo "Starting Helloworld failed"
else
  echo "Starting Helloworld succeeded"
fi
